...
class LoadData():
    ...
    def pre_load_dict(self):
        ...
        cpnum_cellline_map_dict = np.load('.' + dir_opt + '/filtered_data/cpnum_cellline_map_dict.npy', allow_pickle='TRUE').item()
        ...
        return drug_map_dict, cellline_map_dict, cpnum_cellline_map_dict, drug_dict, gene_target_num_dict
    ...
    def pre_load_train(self):
        ...
        cpnum_df = pd.read_csv('./datainfo2/filtered_data/cnv_gistic_20191101.csv')
        ...
        return train_input_df, input_num, num_feature, cellline_gene_df, cpnum_df, num_gene, num_pathway
    ...
    def load_train(self, index, upper_index):
        ...
        drug_map_dict, cellline_map_dict, cpnum_cellline_map_dict, drug_dict, gene_target_num_dict = LoadData(dir_opt, RNA_seq_filename).pre_load_dict()
        ...
        for row in train_input_df.iloc[index : upper_index].itertuples():
            ...
            cpnum_cellline_name = cpnum_cellline_map_dict[row[3]]
    ...
    def pre_load_test(self):
        ...
        gene_pathway_matrix = np.load('.' + dir_opt + '/filtered_data/gene_pathway_matrix.npy')
        all_gene, num_pathway =  gene_pathway_matrix.shape
        return test_input_df, input_num, num_feature, cellline_gene_df, cpnum_df, num_gene, num_pathway
    ...
    def load_test(self):
        ...
        dir_opt = self.dir_opt
        RNA_seq_filename = self.RNA_seq_filename
        test_input_df, input_num, num_feature, cellline_gene_df, cpnum_df, num_gene, num_pathway = LoadData(dir_opt, RNA_seq_filename).pre_load_test()
        drug_map_dict, cellline_map_dict, cpnum_cellline_map_dict, drug_dict, gene_target_num_dict = LoadData(dir_opt, RNA_seq_filename).pre_load_dict()
        target_index_list = gene_target_num_dict.values()
        drug_target_matrix = np.load('.' + dir_opt + '/filtered_data/drug_target_matrix.npy')
    ...
    def pre_load_all(self):
        ...
        gene_pathway_matrix = np.load('.' + dir_opt + '/filtered_data/gene_pathway_matrix.npy')
        all_gene, num_pathway =  gene_pathway_matrix.shape
        return all_input_df, input_num, num_feature, cellline_gene_df, cpnum_df, num_gene, num_pathway
...
if __name__ == "__main__":
    ...
    batch_size = 256
    xTr, yTr, xTe, yTe, x, y = LoadData(dir_opt, RNA_seq_filename).load_all(batch_size, post_data_path)